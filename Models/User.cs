...
namespace ExamenORM.Models
{
    public class User
    {
        ...
        [RegularExpression("(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)(?=.*[@#$%^&+=!]).*$", ErrorMessage = "It does not comply with the allowed format. where an uppercase, lowercase, number, and special symbol(@#$%^&+=!) are required")]
    ...
    public class UniqueEmailAttribute : ValidationAttribute
    {
        ...
        )
        {
            ...
            {
                //Obtenemos la propiedad Id suponiendo que todos los que usen esta validaci�n tienen esta propiedad.
